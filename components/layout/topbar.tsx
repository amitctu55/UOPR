      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div className="relative">
          ...
          >
            ...
            {unread > 0 && (
              <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-alert-500 ring-2 ring-surface-raised" />
            )}
          </Button>
          {notifOpen && (
              ...
              <ul>
                {messages.slice(0, 3).map((m) => (
                  <li key={m.id} className="border-b border-line last:border-0">
                    ...
                    >
                      <p className="text-body-sm font-semibold text-ink">{m.from}</p>
                      <p className="text-body-sm text-ink-muted line-clamp-1">{m.preview}</p>
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <div className="relative">
          <button
            type="button"
            className={cn(
              "flex items-center gap-2 rounded-xl px-2 py-1.5 transition-colors hover:bg-surface-muted focus-visible:shadow-focus"
            )}
            onClick={() => {
          ...
          {menuOpen && (
            <div
              role="menu"
              className="absolute right-0 mt-2 w-52 overflow-hidden rounded-2xl border border-line bg-surface-raised shadow-lift animate-fade-up"
            >
              <Link
                href="/dashboard/settings"
                className="block px-4 py-2.5 text-body-sm hover:bg-surface-muted"
                role="menuitem"
              ...
              <button
                type="button"
                role="menuitem"
                className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-body-sm text-alert-600 hover:bg-alert-50"
                onClick={() => {
                  logout();
                  router.push("/login");
                }}
