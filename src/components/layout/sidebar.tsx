...
export function Sidebar({ role }: { role: UserRole }) {
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "group flex items-center gap-3 rounded-xl px-3 py-2.5 text-body-sm font-medium transition-all duration-200",
              active
                ? "bg-brand-600 text-white shadow-soft dark:bg-brand-500 dark:text-brand-900"
                : "text-ink-muted hover:bg-surface-muted hover:text-ink"
            )}
          >
            <Icon
              className={cn("h-[18px] w-[18px] shrink-0 transition-transform duration-200 group-hover:scale-110")}
              strokeWidth={1.75}
            />
            {item.label}
          </Link>
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-brand-900/40 lg:hidden" onClick={() => setOpen(false)} />
      )}

      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-line bg-surface-raised transition-transform duration-300 lg:static lg:translate-x-0",
