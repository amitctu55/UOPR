...
export default function DoctorDashboard() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title={`Dr. ${user?.name?.replace("Dr. ", "") || "Clinician"}`}
        description="Today’s schedule, telemedicine queue, and patient list."
        actions={
          <Link href="/dashboard/ehr/p-1">
            <Button>
      ...
      <div className="mb-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        <StatBlock label="Appointments today" value={String(schedule.length)} accent="brand" />
        <StatBlock label="Telemedicine queue" value={String(queue.length)} hint="Waiting or confirmed" accent="care" />
        <StatBlock label="Unsigned notes" value="2" hint="Due before EOD" accent="warn" />
        <StatBlock label="Active patients" value={String(patients.length)} accent="brand" />
      </div>
        ...
        <SectionPanel
          ...
          action={
            <Link href="/dashboard/telemedicine/apt-1">
              <Button size="sm">
                <Video className="h-4 w-4" /> Launch
              </Button>
            </Link>
          }
        >
              ...
              <li key={a.id} className="flex items-center justify-between gap-3 rounded-xl bg-surface px-3 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-body-sm text-ink-muted">{formatTime(a.start)}</p>
                  </div>
                </div>
                <Badge tone={statusTone(a.status)}>{a.status}</Badge>
              </li>
      ...
      <SectionPanel className="mt-6" title="Patient list" description="Recent and assigned">
          ...
          <table className="w-full min-w-[640px] text-left text-body-sm">
            <thead>
              <tr className="border-b border-line text-label text-ink-soft">
                ...
                <th className="pb-3 font-semibold">MRN</th>
                ...
                <tr key={p.id} className="border-b border-line last:border-0">
                  ...
                  <td className="py-3">
                    <div className="flex gap-2">
                      <Link href={`/dashboard/patients/${p.id}`} className="font-semibold text-brand-600 hover:underline">
                        Profile
                      </Link>
                      <Link href={`/dashboard/ehr/${p.id}`} className="font-semibold text-brand-600 hover:underline">
                        Chart
                      </Link>
                    </div>
                  </td>
